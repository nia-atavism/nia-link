...
async def final_domain_check():
    ...
    print("Checking domain status on Railway...")
    result = await executor.interact(url=url, actions=actions, account_id="railway-nia")
    
    if result["status"] == "success":
        with open("domain_check_final.png", "wb") as f:
            f.write(base64.b64decode(result["screenshot_base64"]))
        print("Final status screenshot captured.")
    else:
        print(f"Failed: {result.get('message')}")

if __name__ == "__main__":
    asyncio.run(final_domain_check())
